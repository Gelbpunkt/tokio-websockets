...
NUM_WARMUPS = 2
# Number of garbage lines load_test produces
NUM_GARBAGE = 2
...
def run_benchmarks():
    for case in cases:
        ...
        for target in targets:
            ...
            try:
                ...
                for line in client.stdout:
                    if not line:
                        break
                    count += 1
                    if count > NUM_GARBAGE + NUM_WARMUPS:
                        logs.append(line)
                        if count == NUM_GARBAGE + NUM_WARMUPS + NUM_SAMPLES:
                            break
            ...
            lines = [line for line in logs if line.startswith("Msg/sec")]
